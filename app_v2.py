...
st.markdown("""
    <style>
    [data-testid="stMetricValue"] { font-size: 2rem !important; font-weight: 700 !important; color: #1E3A8A !important; }
    [data-testid="stMetricLabel"] { font-size: 0.9rem !important; text-transform: uppercase !important; letter-spacing: 0.5px !important; color: #4B5563 !important; }
    div.stDataFrame { border: 1px solid #E5E7EB !important; border-radius: 8px !important; }
    .css-17eq0hr { background-color: #F9FAFB !important; }
    </style>
...
if st.session_state.logged_in:
    ...
    page = st.sidebar.radio(
        "Navigation",
        ["Dashboard", "Upload Center", "Profit Analysis", "Inventory", "Ads Analytics", "Market & Competitor Insights", "AI Insights", "SaaS Account & Billing"]
    )
    ...
    # --- SMART UPLOAD CENTER ---
        ...
        if uploaded_file:
            ...
            column_mapping = {'sales': 'revenue', 'turnover': 'revenue', 'total_sales': 'revenue', 'gross_sales': 'revenue', 'quantity': 'units_sold', 'qty': 'units_sold', 'items_sold': 'units_sold', 'volume': 'units_sold', 'earnings': 'profit', 'net_profit': 'profit', 'inventory': 'stock', 'marketing': 'ad_spend'}
            df.rename(columns=column_mapping, inplace=True)
            
            if 'product' not in df.columns: st.error("❌ Critical Error: Missing 'product' column."); st.stop()
            for col in ['revenue', 'units_sold', 'profit', 'stock', 'ad_spend']:
                if col not in df.columns: df[col] = 0.0 if col in ['revenue', 'profit', 'ad_spend'] else 0
    ...
    # --- MARKET & COMPETITOR INSIGHTS ---
        ...
        if not df.empty:
            ...
            for _, row in df.iterrows():
                if row["market_demand_index"] > 5.0: st.success(f"🔥 **HIGH MARKET DEMAND:** Acceleration vector for **{row['product']}** is strong. Restock immediately.")
                elif row["market_demand_index"] < 1.5: st.error(f"💀 **DEAD INVENTORY RISK:** **{row['product']}** consumer interest pull has gone cold.")
                else: st.info(f"📋 **{row['product']}** consumer market interest is standard and stable.")
        else:
    ...
    # --- ACCOUNT & BILLING PANEL ---
        ...
        with sub_col1:
            ...
            with tier_col1:
                ...
                if st.button("Upgrade to Growth Core", key="upgrade_growth_btn"): 
                    st.success("Initializing secure payment webhook simulation!")
            with tier_col2:
                ...
                if st.button("Contact Sales", key="upgrade_enter_btn"): 
                    st.info("Corporate connection logged successfully.")
        with sub_col2:
